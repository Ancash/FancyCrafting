...
import de.ancash.fancycrafting.gui.handler.DefaultAutoRecipeMatcherHandler;
import de.ancash.fancycrafting.gui.handler.DefaultRecipeMatchHandler;
import de.ancash.fancycrafting.gui.handler.DefaultRecipePermissionHandler;
...
public class CraftingWorkspaceGUI extends AbstractCraftingWorkspace {
		...
		synchronized (super.lock) {
			...
			for(int y = 0; y<shaped.getHeight(); y++) {
				for(int x = 0; x<shaped.getWidth(); x++) {
					...
					int slot;
					if(isMirrored)
						slot = template.getSlots().getCraftingSlots()[base + y * template.getDimension().getWidth() + (matrix.getWidth() - x - 1)];
					else
	...
	/**
	 * Only checks whether the items are null or not
	 * Since all ingredients in a vanilla recipe 
	 * amount to one a proper check is not required
	 * 
	 * @param recipe
	 * @return
	 */
	private boolean canBeMirrored(IShapedRecipe recipe) {
		...
		for(int y = 0; y<recipe.getHeight(); y++) {
			for(int x = 0; x<recipe.getWidth(); x++) {
				IItemStack a = recipe.getIngredientsArray()[y * recipe.getWidth() + x];
				IItemStack b = matrix.getArray()[y * recipe.getWidth() + (recipe.getWidth() - x - 1)];
				if((a != null) != (b != null))
					return false;
			}
		}
