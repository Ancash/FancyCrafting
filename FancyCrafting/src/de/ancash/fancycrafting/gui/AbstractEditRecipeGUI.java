...
public abstract class AbstractEditRecipeGUI extends IGUI {
	...
	public AbstractEditRecipeGUI(FancyCrafting pl, Player player, IRecipe recipe) {
		this(pl, player, recipe, pl.getWorkspaceObjects().getEditRecipeTitle());
	}

	public AbstractEditRecipeGUI(FancyCrafting pl, Player player, IRecipe recipe, String title) {
		super(player.getUniqueId(), 27, title.replace("%recipe%", recipe.getRecipeName())); //$NON-NLS-1$
	...
	protected final void openMainMenu() {
		...
		newInventory(title, 27);
		for (int i = 0; i < getSize(); i++)
			setItem(plugin.getWorkspaceObjects().getBackgroundItem().getOriginal(), i);

		addInventoryItem(new InventoryItem(this, plugin.getWorkspaceObjects().getDeleteItem().getOriginal(), 18, (a, b,
				c, top) -> Optional.ofNullable(top ? this : null).ifPresent(AbstractEditRecipeGUI::onRecipeDelete)));
		addInventoryItem(new InventoryItem(this, plugin.getWorkspaceObjects().getSaveItem().getOriginal(), 26,
				(a, b, c, top) -> Optional.ofNullable(top && isRecipeValid() ? this : null)
						.ifPresent(AbstractEditRecipeGUI::onRecipeSave)));
		addInventoryItem(new InventoryItem(this, plugin.getWorkspaceObjects().getCloseItem().getOriginal(), 22,
	...
	private void addManageIngredients() {
		addInventoryItem(new InventoryItem(this,
				IngredientsInputGUI.getManageIngredientsItem(plugin, ingredients, shaped,
