...
public abstract class AbstractRecipeViewGUI extends IGUI {
	...
	public AbstractRecipeViewGUI(FancyCrafting pl, Player player, IRecipe recipe) {
		this(pl, player, recipe, pl.getWorkspaceObjects().getViewRecipeTitle());
	}

	@SuppressWarnings("nls")
	public AbstractRecipeViewGUI(FancyCrafting pl, Player player, IRecipe recipe, String title) {
	...
	public void onInventoryClick(InventoryClickEvent event) {
		...
		Set<IRecipe> r = plugin.getRecipeManager().getRecipeByResult(ii);
		if (r == null)
			return;
		r = r.stream().filter(i -> FancyCrafting.canCraftRecipe(i, player)).collect(Collectors.toSet());
		if (r.isEmpty())
			return;
