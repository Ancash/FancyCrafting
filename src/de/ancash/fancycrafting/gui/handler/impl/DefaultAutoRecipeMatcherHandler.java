...
public class DefaultAutoRecipeMatcherHandler implements IAutoRecipeMatcherHandler {
	...
	public DefaultAutoRecipeMatcherHandler(FancyCrafting pl, AbstractCraftingWorkspace workspace,
	...
	private void addQuickCraftingItem(IRecipe recipe, int pos) {
		workspace.addInventoryItem(new InventoryItem(workspace, recipe.getResult(),
				workspace.getTemplate().getSlots().getAutoCraftingSlots()[pos], new Clickable() {
	...
	private void onAutoMatchFinish0() {
		...
		for (i = 0; i < workspace.getTemplate().getSlots().getAutoCraftingSlots().length
			...
			workspace.addInventoryItem(new InventoryItem(workspace, recipe.getResult(),
					workspace.getTemplate().getSlots().getAutoCraftingSlots()[i], new Clickable() {
